...
class ShopXpert_Default_Data{
    ...
    public function body_class( $classes ){
        ...
        if( $post_type == 'elementor_library' || $post_type == 'shopxpert-template' ){

            parse_str( $_SERVER['QUERY_STRING'], $query_str_arr );
            if( !empty( $query_str_arr['elementor-preview'] ) ){

                $tmpType  = ( get_post_type() === 'shopxpert-template' ) ? get_post_meta( $query_str_arr['elementor-preview'], 'shopxpert_template_meta_type', true ) : '';
                ...
                if( !empty( $tmpType ) ){
                    ...
                    $cart_page_id      = shopxpert_Template_Manager::instance()->get_template_id( 'productcartpage' );
                    $checkout_page_id  = shopxpert_Template_Manager::instance()->get_template_id( 'productcheckoutpage' );
            ...
            $classes[] = 'shopxpert-woocommerce-builder';
        }
        return $classes;
    }
    ...
    public function default( $addons = '', $settings = array() ){
        ...
        if( get_post_type() == 'product' ){
            ...
            if( function_exists('shopxpert_get_last_product_id') ){
                self::$product_id = shopxpert_get_last_product_id();
            ...
            switch ( $addons ){
                ...
                case 'wl-single-product-price':
                    ...
                    if( !empty( $product->get_price_html() ) ){
                        ...
                        echo '<p>'.esc_html__('Price does not set this product.','shopxpert').'</p>';
                    }
                    return ob_get_clean();
                ...
                case 'wl-single-product-short-description':
                    ...
                    if ( empty( $short_description ) ) { 
                        echo '<p>'.esc_html__('The short description does not set this product.','shopxpert').'</p>'; 
                    }else{
                        ?>
                ...
                case 'wl-single-product-description':
                    ...
                    if ( empty( $description ) ) { 
                        echo '<p>'.esc_html__('The description does not set this product.','shopxpert').'</p>'; 
                    }else{
                ...
                case 'wl-single-product-rating':
                    ...
                    if ( 'no' === get_option( 'woocommerce_enable_review_rating' ) ) {
                        echo '<div class="wl-nodata">'.esc_html__('Rating does not enable.','shopxpert').'</div>';
                    }
                        ...
                        <div class="product">
                            <div class="woocommerce-product-rating">
                                ...
                                <?php if ( comments_open( self::$product_id ) ) : ?>
                                    <?php //phpcs:disable ?>
                                    <a href="#reviews" class="woocommerce-review-link" rel="nofollow">(<?php printf( _n( '%s customer review', '%s customer reviews', $review_count, 'shopxpert' ), '<span class="count">' . esc_html( $review_count ) . '</span>' ); ?>)</a>
                                    <?php // phpcs:enable ?>
                                <?php endif ?>
                            </div>
                        </div>
                    <?php else:?>
                        <?php echo '<div class="wl-nodata">'.esc_html__('No Rating Available','shopxpert').'</div>';?>
                    <?php endif;
                    return ob_get_clean();
                                ...
                                <figure class="woocommerce-product-gallery__wrapper">
                                    <?php
                                        if ( $product->get_image_id() ) {
                                            ...
                                            $html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'shopxpert' ) );
                                            $html .= '</div>';
                                        }
                        ...
                        <div class="product">
                            <div class="product_meta">
                                ...
                                <?php if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) : ?>
                                    ...
                                    <span class="sku_wrapper"><?php esc_html_e( 'SKU:', 'shopxpert' ); ?> <span class="sku"><?php echo ( $sku = $product->get_sku() ) ? esc_html($sku) : esc_html__( 'N/A', 'shopxpert' ); ?></span></span>

                                <?php endif; ?>

                                <?php echo wc_get_product_category_list( $product->get_id(), ', ', '<span class="posted_in">' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'shopxpert' ) . ' ', '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>

                                <?php echo wc_get_product_tag_list( $product->get_id(), ', ', '<span class="tagged_as">' . _n( 'Tag:', 'Tags:', count( $product->get_tag_ids() ), 'shopxpert' ) . ' ', '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                ...
                case 'wl-single-product-sku':
                    ...
                    if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) : ?>
                        <div class="shopxpert_product_sku_info">
                            <span class="sku-title"><?php esc_html_e('SKU:', 'shopxpert'); ?></span>
                            <span class="sku"><?php echo ( $sku = $product->get_sku() ) ? esc_html($sku) : esc_html__( 'N/A', 'shopxpert' ); ?></span>
                        </div>
                    <?php endif;
                ...
                case 'wl-single-product-tags':
                    ...
                    if( has_term( '', 'product_tag', $product->get_id() ) ) {
                        ?>
                            <div class="shopxpert_product_tags_info">
                                <span class="tags-title"><?php echo sprintf( _n( '%s', '%s', count( $product->get_tag_ids() ), 'shopxpert' ), esc_html__('Tag:','shopxpert'), esc_html__('Tags:','shopxpert') ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                        ...
                        echo esc_html__('Product tags does not exists.', 'shopxpert');
                    }
                ...
                case 'wl-single-product-categories':
                    ...
                    if( has_term( '', 'product_cat', $product->get_id() ) ) {
                        ?>
                            <div class="shopxpert_product_categories_info">
                                <span class="categories-title"><?php echo sprintf( _n( '%s', '%s', count( $product->get_category_ids() ), 'shopxpert' ), esc_html__('Category:','shopxpert'), esc_html__('Categories:','shopxpert') ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span> 
                        ...
                        echo esc_html__('Product category does not exists.', 'shopxpert');
                    }
                ...
                case 'wl-product-data-tabs':
                    ...
                    if( get_post_type() == 'elementor_library' || get_post_type() == 'shopxpert-template' ){
                ...
                case 'wl-single-product-stock':
                    ...
                    if( !empty( $availability['availability'] ) ){
                        ...
                        echo '<p>'.esc_html__('Stock availability does not exist this product.','shopxpert').'</p>';
                    }
                    return ob_get_clean();
                ...
                case 'wl-single-product-upsell':
                    ...
                    if( $product->get_upsell_ids() ){
                        ...
                        echo '<p>'.esc_html__('No upsell products are available.','shopxpert').'</p>';
                    }
                ...
                case 'wl-product-related':
                    ...
                    if( wc_get_related_products( $product->get_id(), $args['posts_per_page'], $product->get_upsell_ids() ) ){
                        ...
                        echo '<p>'.esc_html__('No related products are available.','shopxpert').'</p>';
                    }
