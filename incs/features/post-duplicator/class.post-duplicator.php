...
class ShopXpert_Post_Dupicator{
    /**
     * [$_instance]
     * @var null
     */
     ...
     * @return [ShopXpert_Post_Dupicator]
     */
    ...
    public function __construct(){
        add_filter( 'admin_action_shopxpert_duplicate_post_as_draft', [ $this, 'duplicate' ] );
    ...
    public function row_actions( $actions, $post ){
        ...
        $enable = !empty( shopxpert_get_option( 'postduplicate_condition', 'shopxpert_others_tabs', array('all') ) ) ? shopxpert_get_option( 'postduplicate_condition', 'shopxpert_others_tabs', array('all') ) : array('all');

        if ( current_user_can('edit_posts') && ( in_array( $post->post_type, $enable ) || in_array('all', $enable) ) ) {
            $actionurl = admin_url('admin.php?action=shopxpert_duplicate_post_as_draft&post=' . $post->ID );
            $url = wp_nonce_url( $actionurl, 'shopxpert_duplicate_nonce' );
            $actions['shopxpertduplicate'] = '<a href="'.$url.'" title="'.esc_attr__( 'HT Duplicator', 'shopxpert' ).'" rel="permalink">'.esc_html__( 'SX Duplicate', 'shopxpert' ).'</a>';
    ...
    public function duplicate(){
        ...
        if (! ( isset( $_REQUEST['post']) || isset( $_REQUEST['post'] )  || ( isset( $_REQUEST['action'] ) && 'shopxpert_duplicate_post_as_draft' == $_REQUEST['action'] ) ) ) {
         ...
         * Nonce verification
         */
        if( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'shopxpert_duplicate_nonce' ) ) {
            return; 
        }
        ...
        */
        if ( !current_user_can( 'edit_post', $post_id ) ) {
            wp_die( esc_html__( 'You do not have permission to duplicate this post!','shopxpert' ) );
        }
        ...
        */
        if ( post_password_required( $post )  && ! current_user_can( 'edit_post_passwords' ) ) {
            wp_die( esc_html__( 'You do not have permission to duplicate this password-protected post!', 'shopxpert' ) );
        }
        ...
        */
        if ($post->post_status === 'private') {
            wp_die( esc_html__('You do not have permission to duplicate this private post!', 'shopxpert') );
        }
        ...
        if ( isset( $post ) && $post != null) {
            ...
            if( ! is_wp_error( $new_post_id ) ) {
                ...
                $meta_data_cache_key = 'shopxpert_fetch_meta_data_'.$post_id;
...
ShopXpert_Post_Dupicator::instance();