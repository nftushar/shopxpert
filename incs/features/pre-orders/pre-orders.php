...
class Shopxpert_Pre_Orders{
    ...
    public function __construct(){
        ...
        Shopxpert_Pre_Order_AddTo_Cart::get_instance();

        // Price manager
        Shopxpert_Pre_Order_Price::get_instance();

        // Pre-Order Content manager
        Shopxpert_Pre_Order_Content::get_instance();

        // Order manager
        Shopxpert_Pre_Order_Place::get_instance();

        // Update schedule manager
        add_action('_shopxpert_pre_order_schedule_date_cron',[ $this, 'schedule_date_cron' ] );
    ...
    public function get_pre_order_status( $product_id ){
        ...
        $pre_order_date = $this->get_saved_data( $product_id, 'shopxpert_pre_order_available_date', 'shopxpert_pre_order_available_date' );
        $pre_order_time = $this->get_saved_data( $product_id, 'shopxpert_pre_order_available_time', 'shopxpert_pre_order_available_time' );
        ...
        if( ( get_post_meta( $product_id, 'shopxpert_pre_order_enable', true ) === 'yes' ) && ( $remainingTime > 0 ) ){
            return true;
        }else{
            return false;
        }
    ...
    public function delete_product_transients( $product_id ){
        $status = get_post_meta( $product_id, 'shopxpert_pre_order_enable', true );
        if( $status === 'yes' ){
    ...
    public function has_in_cart(){
        ...
        if ( WC()->cart ) {
            foreach ( WC()->cart->get_cart() as $key => $cart_item ) {
                if ( isset( $cart_item['shopxpert_pre_order'] ) && $cart_item['shopxpert_pre_order'] === true  ) {
                    $has_in_cart = true;
                    break;
                }
            }
    ...
    public function get_calculate_price( $product_id, $product_price ) {
		...
		$manage_price = $this->get_saved_data( $product_id, 'shopxpert_pre_order_manage_price', 'shopxpert_pre_order_manage_price', 'product_price' );
		$amount_type  = $this->get_saved_data( $product_id, 'shopxpert_pre_order_amount_type', 'shopxpert_pre_order_amount_type', 'percentage' );
		$amount       = (float)$this->get_saved_data( $product_id, 'shopxpert_pre_order_amount', 'shopxpert_pre_order_amount', '50' );
...
Shopxpert_Pre_Orders::get_instance();