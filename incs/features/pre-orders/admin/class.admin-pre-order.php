...
class Shopxpert_Admin_Pre_Orders extends Shopxpert_Pre_Orders{
    ...
    public function posts_custom_column( $column_name, $post_id ){
        ...
        if ( 'shopxpert_pre_order' === $column_name ) {
            if ( $this->get_pre_order_status( $post_id ) ) {

                $pre_order_date = $this->get_saved_data( $post_id, 'shopxpert_pre_order_available_date', 'shopxpert_pre_order_available_date' );
                $pre_order_time = $this->get_saved_data( $post_id, 'shopxpert_pre_order_available_time', 'shopxpert_pre_order_available_time' );
                ...
                if ( $remainingTime > 0 ) {
                    if ( $remainingTime > 86400 ) {
                        ...
                        echo '<div class="shopxpert-pre-order-after" style="color:#ec5858;">' . esc_html( $due_time ) . '</div>';
                    } else {
    ...
    public function has_pre_order_item( $product_id, $order_created_timestamp ){
        ...
        if( ( get_post_meta( $product_id, 'shopxpert_pre_order_enable', true ) === 'yes' ) && ( $pre_order_date_timestamp >= $order_created_timestamp ) ){
            return true;
        }else{
            return false;
        }
