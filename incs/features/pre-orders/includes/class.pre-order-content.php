...
class Shopxpert_Pre_Order_Content extends Shopxpert_Pre_Orders{
    ...
    public function get_item_data( $item_data, $cart_item ){
        ...
        if ( $this->get_pre_order_status( $product_id ) ) {
            $item_data[] = array(
                'name'      => 'shopxpert_cart_availability',
                'display'   => $this->availability_date( $product_id ),
                'value'     => 'wc_shopxpert_pre_order_content',
            );
        }
    ...
    public function pre_order_content( $product_id ){
        ...
        $availability_date      = $this->replace_placeholder( '{availability_date}', '<span class="shopxpert-availability-date">'.$date_format.'</span>', $availability_date_text );
        $availability_date      = $this->replace_placeholder( '{availability_time}', '<span class="shopxpert-availability-time">'.$time_format.'</span>', $availability_date );
        ...
        ob_start();
        ?>
            <div class="shopxpert-pre-order-area">
                <?php
                    if ( $remainingTime > 0 ) {
                        echo '<div class="shopxpert-availability-date-area"><div class="shopxpert-availability-date-inner">'.$availability_date.'</div></div>';
                    }

                    if( $show_countdown === 'on' && ! empty( $pre_order_date ) ){
                        ?>
                            <div class="shopxpert-pre-order-countdown" data-countdown="<?php echo esc_attr( $remainingTime ); ?>" data-customlavel='<?php echo wp_json_encode( $data_customlavel ) ?>'></div>
                        <?php
                    }
                ?>
            </div>
    ...
    public function availability_date( $product_id ){
        ...
        if ( $this->get_pre_order_status( $product_id ) ) {
            ...
            $pre_order_date_content = '<div class="shopxpert-pre-order-availability-date-cart">'.$availability_date.'</div>';
