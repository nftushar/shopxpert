...
<div class="wl-suggest-price">
    ...
    if (isset($_REQUEST['wlsubmit-' . $id])) {
        ...
        if (!isset($_POST['shopxpert_suggest_price_nonce_field']) || !wp_verify_nonce($_POST['shopxpert_suggest_price_nonce_field'], 'shopxpert_suggest_price_action')) {
            echo '<p class="wlsendmessage">' . esc_html__('Sorry, your nonce verification failed.', 'shopxpert') . '</p>';
        } else {
            // Use wp_unslash before sanitization
            $name = isset($_POST['wlname']) ? wp_unslash($_POST['wlname']) : '';
            $email = isset($_POST['wlemail']) ? wp_unslash($_POST['wlemail']) : '';
            $message = isset($_POST['wlmessage']) ? wp_unslash($_POST['wlmessage']) : '';
            ...
            $subject = esc_html__("Suggest Price For - " . $product->get_title(), 'shopxpert');
            $headers = esc_html__('From: ', 'shopxpert') . esc_html($email) . "\r\n" . esc_html__('Reply-To: ', 'shopxpert') . esc_html($email) . "\r\n";
    ...
    ?>
    <button id="wlopenform-<?php echo esc_attr($id); ?>" class="wlsugget-button wlopen"><?php echo esc_html__($settings['openButtonText'], 'shopxpert'); ?></button>
    <button id="wlcloseform-<?php echo esc_attr($id); ?>" class="wlsugget-button wlclose" style="display: none;"><?php echo esc_html__($settings['closeButtonText'], 'shopxpert'); ?></button>
...
<script type="text/javascript">
    ;jQuery(document).ready(function($) {
        ...
        var target_form = 'form#wlsuggestform-<?php echo esc_attr($id); ?>';
        $(open_formbtn).on('click', function() {
            $(this).hide();
            $(this).siblings(close_formbtn).show();
            $(this).siblings(target_form).slideDown('slow');
        });
        ...
        $(close_formbtn).on('click', function() {
            ...
            $(this).siblings(target_form).slideUp('slow');
        });
