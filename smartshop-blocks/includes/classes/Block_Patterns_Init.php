...
class Block_Patterns_init {
    ...
    public function register_patterns_category(){
        register_block_pattern_category( 'shoplentor', [ 'label' => __( 'ShopLentor', 'smartshop' ) ] );
    }
    ...
    public function prepare_pattern_data( $pattern ){
        ...
        if ( empty( $pattern_info['slug'] ) ) {
            _doing_it_wrong(
                ...
                esc_html( sprintf( __( 'The block pattern fails to register because the "Slug" field is missing in this pattern "%s"', 'smartshop' ), $pattern_info['title'] ) ),
                '6.0.0'
            );
            return;
        }
        ...
        if ( empty( $pattern_info['title'] ) ) {
            _doing_it_wrong(
                'register_block_patterns',
                esc_html( sprintf( __( 'The block pattern fails to register because the "Title" field is missing in this pattern "%s"', 'smartshop' ), $pattern_info['title']) ),
                '6.0.0'
            );
            return;
        }
