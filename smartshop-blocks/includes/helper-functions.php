...
function smartshopBlocks_compare_button( $button_arg = array() ){
    ...
    if( class_exists('Ever_Compare') || class_exists('Smartshop_Ever_Compare') ){
        ...
        if( $button_style == 1 ){
            ...
            $output = '<a title="'. esc_attr__('Add to Compare', 'smartshop') .'" href="'. esc_url( $comp_link ) .'" class="smartshop-compare compare" data-product_id="'. esc_attr( $product_id ) .'" rel="nofollow">'.esc_html__( 'Compare', 'smartshop' ).'</a>';
        }
        return $output;
    }else{
        return 0;
